...
After deployment completes, the output will show the application URL:
...
1. Check that the managed identity has been granted SQL access:
   ```powershell
   az role assignment list --assignee <MANAGED_IDENTITY_PRINCIPAL_ID>
   ```

2. Verify federated credential is configured:
   ```powershell
